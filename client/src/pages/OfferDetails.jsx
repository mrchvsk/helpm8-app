...
export default function OfferDetails() {
    ...
    useEffect(() => {
        const fetchOfferDetails = async () => {
            try {
                ...
                if (!response.ok) {
                    throw new fetchError('Offer not found');
                }
    ...
    if (fetchError) {
        return <div className="text-red-600 text-center mt-4">{fetchError}</div>;
    }
    ...
    const handleParticipate = async () => {
        try {
            const response = await fetch(`/offers/${id}/part`, {
                method: 'PUT',
                headers: {
            ...
            if (!response.ok) {
                throw new fetchError('Failed to participate in the offer');
            }
        } catch (fetchError) {
            console.fetchError('fetchError:', fetchError);
        }
    };
